/********************************************************************************
*   Ledger Node JS API for MyFactomWallet
...
var LedgerFct = function(comm) {
  ...
  if ( this.comm.timeout < 1000 ) {
    this.comm.timeout = 1000
  }
  //console.log(comm)
}

LedgerFct.prototype.close_async = function () {
  return this.comm.close_async().then(function() {
...
LedgerFct.prototype.getAddress_async = function(path, boolDisplay, boolChaincode) {
  ...
  return this.comm.exchange(buffer.toString('hex'), [0x9000]).then(function(response) {
    ...
    if (boolChaincode) {
      result['chainCode'] = response.slice(1 + publicKeyLength + 1 + addressLength, 
	                                   1 + publicKeyLength + 1 + addressLength + 32).toString('hex')
    }
    return result
  })
}
...
LedgerFct.prototype.signTransaction_async = function(path, tx) {
  ...
  //prepare metadata for confirmation
  offset = 0
  ...
  return utils.foreach(apdus, function(apdu) {
    ...
    var result = {}
    //rcd
    result['r'] = response.slice(0, 33).toString('hex')
    //length of signature (64)
    result['l'] = fctUtil.bufferToInt(response.slice(33, 33 + 2))
    //signature
    result['s'] = response.slice(35, 33 + 2 + result['l'] ).toString('hex')
